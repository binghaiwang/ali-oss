...
//Cross-Region Replication CRUD operation
/**
 * set Bucket Replication
 * @name {String} name bucket name
 * @rules {Array} rules Bucket Replication Rule  now only support one rule
 *  var rules = [{
 *    id: 'rule id',
 *    prefixSet:['prefix_1', 'prefix_2'],
 *    action: 'ALL/PUT/DELETE/ABORT',
 *    destination: {
 ...
 */
proto.putBucketReplication = function* putBucketReplication(name, rules, options) {
  rules = rules || [];

  assert(rules.length, 'rules is required');
  rules.forEach(rule => {
    assert(rule.destination, 'destination is required');
    assert(rule.destination.bucket, 'destination bucket is required');
    assert(rule.destination.location, 'destination location is required');
  });

  var params = this._bucketRequestParams('POST', name, {
    replication: '',
    comp: 'add'
  }, options);
  ...
  for (var i = 0, l = rules.length; i < l; i++) {
    ...
    if (rule.prefixSet) {
      xml += '<PrefixSet>';
      rule.prefixSet.forEach(addPrefix);
      xml += '</PrefixSet>';
    }
    ...
    xml += '<Destination>';
    xml += '<Bucket>' + rule.destination.bucket + '</Bucket>';
    xml += '<Location>' + rule.destination.location + '</Location>';
    xml += '</Destination>';

    if (rule.historicalObjectReplication) {
      xml += '<HistoricalObjectReplication>' + rule.historicalObjectReplication + '</HistoricalObjectReplication>';
    }
